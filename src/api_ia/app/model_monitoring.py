...
class ModelMonitor:
    ...
    def __init__(self):
        ...
        self.model_top1_accuracy = Gauge(
            'model_top1_accuracy', 
            'Top-1 accuracy du modèle (classe prédite correcte)',
            ['model_name']
        )
        
        self.model_top3_accuracy = Gauge(
            'model_top3_accuracy', 
            'Top-3 accuracy du modèle (classe correcte dans top 3)',
            ['model_name']
        )
        
        self.model_confidence_high = Counter(
            'model_confidence_high',
            'Prédictions avec confiance élevée (>0.8)',
            ['model_name']
        )
        
        self.model_confidence_medium = Counter(
            'model_confidence_medium',
            'Prédictions avec confiance moyenne (0.5-0.8)',
            ['model_name']
        )
        
        self.model_confidence_low = Counter(
            'model_confidence_low',
            'Prédictions avec confiance faible (<0.5)',
            ['model_name']
        )
        
        self.model_rejection_rate = Gauge(
            'model_rejection_rate',
            'Taux de rejet (confiance < seuil)',
            ['model_name']
        )
        ...
        self.model_embedding_quality = Gauge(
            'model_embedding_quality',
            'Qualité des embeddings générés',
            ['model_name']
        )
        
        self.model_embedding_normality = Gauge(
            'model_embedding_normality',
            'Normalité des embeddings (test de Shapiro-Wilk)',
            ['model_name']
        )
        
        self.model_feature_activation = Gauge(
            'model_feature_activation',
            'Activité moyenne des features du modèle',
            ['model_name']
        )
        ...
        self.model_drift_score = Gauge(
            'model_drift_score',
            'Score de drift des données d\'entrée',
            ['model_name']
        )
        ...
        self.model_predictions_success = Counter(
            'model_predictions_success',
            'Nombre de prédictions réussies',
            ['model_name']
        )
        
        self.model_predictions_failed = Counter(
            'model_predictions_failed',
            'Nombre de prédictions échouées',
            ['model_name']
        )
        ...
        self.model_throughput = Gauge(
            'model_throughput',
            'Débit de prédictions par minute',
            ['model_name']
        )
    ...
    def update_prediction_metrics(
        ...
        try:
            ...
            logger.debug(f"Métriques mises à jour - Prédictions: {self.prediction_count}, "
    ...
    def _calculate_feature_activation(self, embedding: np.ndarray) -> float:
        ...
        try:
            ...
            activation_score = (
                0.6 * min(variance / 5.0, 1.0) +  # Variance
                0.4 * (1.0 - sparsity)  # Non-sparsité
            )
    ...
    def _generate_reference_data(self, embedding: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            'embedding_features': np.random.normal(0, 1, (100, embedding.shape[0]))
        }
    ...
    def get_model_health_status(self) -> Dict[str, Any]:
        ...
        try:
            avg_inference_time = (
                self.total_inference_time / max(self.prediction_count, 1)
            )
            ...
            avg_embedding_quality = (
                np.mean(self.embedding_qualities) if self.embedding_qualities else 0.0
            )
