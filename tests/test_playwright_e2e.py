...
class TestAuthentification:
    ...
    def test_connexion_utilisateur(self, page: Page):
        ...
        if signup_button.count() > 0:
            ...
            # Vérifier que les positions sont différentes
            assert (
                login_button_position != signup_button_position
            ), "Le bouton de connexion et le bouton d'inscription sont au même endroit"
...
class TestInterfaceUtilisateur:
    ...
    def test_deconnexion(self, page: Page):
        ...
        try:
            ...
            current_url = page.url
            assert (
                "login" in current_url or "connexion" in current_url or current_url == "https://engravedetect.fr"
            ), "Pas redirigé vers la page de connexion"
    ...
    def test_actualisation_donnees(self, page: Page):
        ...
        refresh_button = page.locator(
            'button:has-text("🔄 Actualiser les données"), button:has-text("🔄"), button[title*="actualiser"], button[title*="refresh"]'
        )
...
class TestPerformanceInterface:
    ...
    def test_stabilite_interface(self, page: Page):
        ...
        for i in range(3):
            ...
            if canvas_box:
                page.mouse.click(canvas_box["x"] + 50 + i * 20, canvas_box["y"] + 50 + i * 20)
                page.mouse.down()
                page.mouse.move(canvas_box["x"] + 100 + i * 20, canvas_box["y"] + 100 + i * 20)
                page.mouse.up()
